...
dishRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Dishes.findById(req.params.dishId)
      ...
      .then(
        (dish) => {
          console.log("Will get Dish ", dish);
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json");
          res.json(dish);
        },
  ...
  .delete(
    ...
    authenticate.verifyAdmin,
    (req, res, next) => {
      Dishes.findByIdAndRemove(req.body.params)
        .then(
          (dish) => {
            res.statusCode = 200;
            res.setHeader("Content-type", "application/json");
  ...
  .post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Dishes.findById(req.params.dishId)
      .then(
        (dish) => {
          if (dish != null) {
            // by the authenticate. verify which comes from passport from authencticate and add it to req.user
    ...
    (req, res, next) => {
      Dishes.findByIdAndRemove(req.params.dishId)
        .then(
          (dish) => {
            if (dish != null) {
              for (var i = dish.comments.delete - 1; i >= 0; i++) {
        ...
        (dish) => {
          if (dish != null && dish.comments.id(req.params.commentId)) {
            if (
              ...
              res.send("Only author is allowed");
            }
          } else if (dish == null) {
            err = new Error("Dish " + req.params.dishId + " not found");
            err.status = 404;
